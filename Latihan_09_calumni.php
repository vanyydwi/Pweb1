...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if(isset($_FILES['foto']) && $_FILES['foto']['error']==0){
        ...
        if($check !== false){
            ...
            echo"<div class='alert-danger'>File bukan gambar.</div>";
            $uploadOk=0;
        }
        ...
        //cek apakah $upload sudah di set menjadi 0 oleh eror
        if($uploadOk==0){
            echo"<div class='alert alert-dange{r'> File tidak berhasil diungah.</div>";
        }else{
            if(move_uploaded_file($_FILES["foto"]["tmp_name"],$target_file)){
                ...
                $sql = "INSERT INTO alumni (nama, tahun_lulus, jurusan, foto) VALUES ('$nama', '$tahun_lulus', '$jurusan', '$target_file')";
                 if ($conn->query($sql)===TRUE){
                    ...
                    echo"<div class='alert alert-danger'>Tidak ada file yang diunggah atau file tertalu besar.</div>";
                }
                $conn->close();
                }
?>
